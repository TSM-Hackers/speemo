...
class RecordWindow(wx.Frame):
    def __init__(self, parent, users=None, record_data=None, title="Conversation"):
        ...
        self.message_display = UserMessageDisplay(
            self.panel, wx.SIMPLE_BORDER | wx.VSCROLL, (900, 500), USERS)
...
class UserDockElement(wx.Panel):
    def __init__(self, parent, user_name):
        ...
        self.user_bitmap = wx.StaticBitmap(self.user_panel, wx.SUNKEN_BORDER |
                                           wx.ID_ANY, self.user_img.ConvertToBitmap())
        ...
        self.hexagon_bitmap = wx.StaticBitmap(
            self.hexagon_panel, wx.ID_ANY, self.hexagon_img.ConvertToBitmap())
...
class UserDockWindow(wx.Frame):
    ...
    def text2sentiment(self, text):
        text2 = [x for x in text.split() if x]
        name, label = st.sample_network(net, text2)
        print(text2, name)
        return label
    ...
    def activate_user(self, message):
        ...
        self.active_user = self.elements[message["user"]][1]
        self.active_user.update_hexagon(self.text2sentiment(message["text"]))
